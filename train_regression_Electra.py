...
def main(config=None):
    try: 
        ...
        dm = DTIpredictDataModule(config.task_name, config.d_model_name, config.p_model_name,
                                 config.num_workers, config.batch_size, config.prot_maxlength, config.drug_maxlength)
        dm.prepare_data()
        dm.setup()
        ...
        if config.model_mode == "train":
            model = DTIpredictModel(config.lr, config.dropout, config.layer_features,
            ...
            for testmodel_type in testmodel_types:
                ...
                model = DTIpredictModel.load_from_checkpoint(model_file)
