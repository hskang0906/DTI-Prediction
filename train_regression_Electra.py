...
def main(config=None):
    try: 
        ...
        checkpoint_callback = ModelCheckpoint(f"{config.task_name}_{model_type}_Electra_regression", monitor="valid_MSE", mode="min")
        ...
        if config.model_mode == "train":
            ...
            for testmodel_type in testmodel_types:
                model_file = f"./log/{config.task_name}_{testmodel_type}_Electra_regression/*.ckpt"
...
if __name__ == '__main__':
    ...
    if run_type == SWEEP:
        config = load_hparams('config/sweep/config_hparam_regression_Electra.json')
        project_name = config["name"]
        sweep_id = wandb.sweep(config, project=project_name)
        wandb.agent(sweep_id, main)
    
    else:
        config = load_hparams('config/config_hparam_regression_Electra.json')
        project_name = config["name"]
        main(config)